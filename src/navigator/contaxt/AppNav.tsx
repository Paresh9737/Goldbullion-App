...
import {NavigationContainer} from '@react-navigation/native';
import ReactNativeBiometrics, {BiometryTypes} from 'react-native-biometrics';
import {AuthContext, AuthContextType} from './AuthContaxt';
import DrawerNavigator from '../DrawerNavigater';
import AuthStackNavigator from '../AuthStackNavigator';
import CustomAlert from '../../components/CustomAlert';
import {ERRORS, MESSAGES} from './constants';
...
const AppNav: React.FC = () => {
  ...
  // New function to handle first-time launch logic
  const checkFirstLaunch = useCallback(async () => {
    try {
      ...
      if (!hasLaunchedBefore) {
        // First launch - show lock screen and mark as launched
    ...
    try {
      ...
      if (success) {
        updateAuthState({
          isSecurityVerified: true,
          alertVisible: false,
          isFirstLaunch: false, // Ensure first launch is marked as complete
        });
      } else {
        updateAuthState({
          alertMessage: MESSAGES.APP_LOCKED,
  ...
  useEffect(() => {
    // Check first launch when component mounts
    checkFirstLaunch();

    // Only set up app state listener if it's the first launch
    if (authState.isFirstLaunch) {
      const subscription = AppState.addEventListener(
        'change',
        (nextAppState: AppStateStatus) => {
          if (nextAppState === 'active' && authState.isFirstLaunch) {
            checkAndAuthenticateUser();
          }
        },
      );
  ...
  }, [checkFirstLaunch, checkAndAuthenticateUser, authState.isFirstLaunch]);

  // Modification to the rendering logic
  if (isLoading || (authState.isFirstLaunch && !authState.isSecurityVerified)) {
    return (
      <>
        <View style={styles.loadingContainer}>
          <Image
